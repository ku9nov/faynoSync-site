...
description: Create a new FaynoSync application record with metadata, visibility settings, and authenticated API access.
---
...
| `download_mode` | string | ❌ | Private apps only: `unlisted` or `strict`, who may [download](./download) artifacts and check for updates. Defaults to `unlisted` when `ENABLE_PRIVATE_APP_DOWNLOADING=true`, otherwise `strict` |
| `tuf` | boolean | ❌ | Enable TUF framework for application |
| `cdn` | boolean | ❌ | Enable or disable API response caching on S3. Not allowed for private apps |
...
- A private app supports only the `manual`, `tauri` and `squirrel_darwin` updaters. Feed-based updaters (`velopack`, `sparkle`, `electron-builder`, `squirrel_windows`) are rejected on [upload](./upload)
- Clients of a `strict` app need a [download token](./download-tokens-regenerate) for update checks and downloads