...
description: Sign FaynoSync application artifacts with TUF metadata and publish trusted update metadata for clients.
---
...
### Publishing Started (202 Accepted)
...
### Nothing to Sign (200 OK)

When every signable artifact of this version is already signed with TUF metadata:
...
| `message` | string | `"Artifact(s) publishing started"` or `"Nothing to sign: all signable artifacts are already signed"` |
...
- Updater feed files (`releases.{channel}.json`, `appcast*.xml`, `RELEASES`, `*.yml`) are not TUF targets and are never signed.
- Artifacts uploaded through [presigned uploads](/docs/presigned-uploads) carry hashes supplied by the uploader. Before signing, publish verifies them against the stored bytes — reading the SHA-256 computed by storage on Garage and AWS S3, or streaming the object once on DigitalOcean Spaces and GCS. If any artifact does not match, no artifact of the version is signed and the task fails with the list of mismatching objects. See [TUF: hashes are verified before signing](/docs/presigned-uploads#tuf-hashes-are-verified-before-signing).
