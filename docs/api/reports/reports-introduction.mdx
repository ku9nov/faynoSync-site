...
- Rate limits are kept in Redis, which every faynoSync instance requires.
...
- **Management API** (`PATCH /reports/groups/:groupHash`, `DELETE /reports/groups/:groupHash`) is also JWT-authenticated but gated by stronger permissions: `edit` for updates and `delete` for deletion. All management endpoints are scoped to the requester's accessible apps.
...
Every group carries a lifecycle `status` used for triage:

- `open` — the default; new groups are created `open`. In list queries, `status=open` also matches pre-existing groups that predate the field (those with no `status`).
- `resolved` — marked as handled. Resolving records `resolved_at` and `resolved_by` (the requester's username).
- `muted` — suppressed noise; muted groups stay suppressed even when new events arrive.

Alongside status, groups support two free-form triage fields that are independent of the lifecycle state: `tags` (up to 20, each `^[a-zA-Z0-9._-]{1,64}$`) and a `note` (up to 2000 characters). Sending an empty tags array or empty note clears the respective field.

### Regression auto-reopen

A new event on a `resolved` group automatically flips it back to `open` and clears the resolution markers (`resolved_at` / `resolved_by`), so a recurring failure resurfaces instead of staying hidden. Tags and note are preserved across the reopen. `muted` groups are never auto-reopened.

Status, tags, and note are set through the [Update Report Group](/docs/api/reports/update-group) endpoint, and groups can be permanently removed with [Delete Report Group](/docs/api/reports/delete-group).
...
- [Update Report Group](/docs/api/reports/update-group)
- [Delete Report Group](/docs/api/reports/delete-group)
