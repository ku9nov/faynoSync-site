...
| `status` | string | Filter by lifecycle status: `open`, `resolved`, or `muted`. `status=open` also matches pre-existing groups that have no status field; `status=all` disables the filter |
...
| `status` | string | Lifecycle status: `open`, `resolved`, or `muted` (new groups are created `open`) |
| `tags` | array | Free-form triage tags (omitted when empty) |
| `note` | string | Free-form triage note (omitted when empty) |
| `resolved_at` | string | When the group was resolved (present only while `resolved`) |
| `resolved_by` | string | Username that resolved the group (present only while `resolved`) |
...
- To change a group's status, tags, or note, use [Update Report Group](/docs/api/reports/update-group); to remove it entirely, use [Delete Report Group](/docs/api/reports/delete-group).
