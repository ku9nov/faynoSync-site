...
description: Check the latest FaynoSync application version and update metadata for supported client update flows.
---
...
|--------|-------|----------|
| `X-Download-Token` | `fnd_…` [download token](../apps/download-tokens-regenerate) of the app and channel | Private apps in `strict` mode |
...
Public apps and private apps in `unlisted` mode need no credential. For a private app in `strict` mode, a request without a valid credential gets exactly the same answer as an app that does not exist, `400 Bad Request`:
...
- **Private Apps**: Download URLs of a private app point to [`/download`](../apps/download), which applies the same `download_mode` check. Responses of private apps are never cached in Redis or published to the CDN
