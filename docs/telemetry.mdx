...
You can enable statistics by defining the variable `ENABLE_TELEMETRY=true` in the `.env` file. When it is `false`, faynoSync records no client telemetry: `/checkVersion` does not count requests and `/telemetry/beacon` returns `403`.
