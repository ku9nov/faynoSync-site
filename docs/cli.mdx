...
The `--updater` flag selects the [updater type](./updaters/index.mdx) the upload targets, so artifacts land in the correct isolated folder and the server generates the right feed files (`RELEASES`, `*.yml`, `releases.{channel}.json`, `appcast.{channel}.xml`). The CLI validates only that the value is a known updater type — all other rules are enforced server-side. When omitted, the platform's default updater is used.
