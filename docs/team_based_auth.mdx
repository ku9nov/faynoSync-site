---
sidebar_position: 6
title: Team Based Authorization
description: Complete guide to FaynoSync team-based access control and permission management
---
# Team Based Authorization Matrix

**Team Based Authorization Matrix** — a system that enables fine-grained, team-level access control. This mechanism allows administrators to create team users, assign specific permissions, and strictly isolate data between different administrators.
...
- **Has no access** to any user-related API ([user management endpoints](./api/team/users-introduction)).
...
- Uploads ([`POST /upload`](./api/apps/upload), [Upload Init](./api/apps/upload-init), [Upload Complete](./api/apps/upload-complete)) need the Upload permission, and the app, channel, platform and architecture of the upload must all be in the team user's allowed lists. Otherwise the request returns `403` before any file is stored. Before v2.4.0 `POST /upload` did not check these lists.
...
- [`GET /checkVersion`](./api/info/latest)

- [`GET /apps/latest`](./api/info/fetch-latest-build)
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Upload artifacts</td>
      <td>✅ (with Upload permission, into allowed apps, channels, platforms and architectures only)</td>
      <td>✅</td>
    </tr>
    <tr>
      <td>Manage users</td>
...
- [Team users introduction](./api/team/users-introduction) — managing team users under an administrator.
- [Update admin permissions](./api/team/updateAdmin) — change a team user's username, password, and permissions.
- [Login](./api/auth/login) — authenticate as an administrator or team user.
- [Whoami](./api/info/whoami) — resolve the current authenticated identity and scope.
