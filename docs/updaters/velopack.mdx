...
FaynoSync builds the feed per request, so it can apply per-client logic the static object can't (object storage ignores query parameters). Package (`*.nupkg`) requests to this route are `302`-redirected to the real storage link.
