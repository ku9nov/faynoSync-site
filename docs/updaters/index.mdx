...
description: Complete guide to FaynoSync update mechanisms — Velopack, Sparkle, Squirrel, Electron Builder, Tauri, and manual updates.
---
...
- [Sparkle (macOS)](./sparkle.mdx) — `sparkle`; faynoSync hosts a materialized appcast, preserving every element verbatim, no SDK.
...
See the [Example Apps](../examples/index.mdx) for runnable desktop integrations: [Electron](../examples/electron.mdx) (`electron-builder`), [Squirrel](../examples/squirrel.mdx) (`squirrel_darwin` / `squirrel_windows`), [Tauri](../examples/tauri.mdx), [Velopack](../examples/velopack.mdx) (`velopack`, no SDK), and [Sparkle](../examples/sparkle.mdx) (`sparkle`, no SDK).
:::
...
:::note
[`velopack`](./velopack.mdx), [`sparkle`](./sparkle.mdx), and [`squirrel_windows`](./squirrel-windows.mdx) do **not** use `/checkVersion`. They expose feed files their clients fetch natively — see their pages for the exact URLs.
:::
