...
| `DeviceID` | string | No | Device identifier sent as `X-Device-ID` header when provided. Required to participate in [staged rollouts](#staged-rollout). |
| `DownloadToken` | string | No | [Download token](../../api/apps/download-tokens-regenerate) of a private app in `strict` mode, sent as the `X-Download-Token` header. Requires SDK v0.4.0 or later. |

For a `strict` private app, a check without `DownloadToken` gets the same answer as an app that does not exist. When `DownloadToken` is set, the SDK skips `EdgeURL` and asks the API directly, because private apps are never published to the edge. If your application downloads the artifact itself, send the same header to [`/download`](../../api/apps/download) and use `faynosync.StripDownloadTokenOnRedirect` as the `http.Client`'s `CheckRedirect`, so the token is not forwarded to the storage host.
...
| `Rollout` | `*RolloutInfo` | Present (non-nil) only when the server offered a staged rollout for the version. See [Staged Rollout](#staged-rollout). |
...
When the install is **not** in the bucket, the SDK forces `UpdateAvailable` to `false` **and clears `UpdateURL`/`PackageURLs`**, so a caller that checks the URLs instead of `UpdateAvailable` still cannot pull an update the device was not offered. A normal `if resp.UpdateAvailable` gate is all you need; the `Rollout` field exposes the decision for logging:
...
| Field | Type | Description |
|-------|------|-------------|
| `Percent` | int | The rollout target percentage the server issued for the version. |
| `Seed` | string | Stable per-version seed used to bucket the device. |
| `Bucket` | `*int` | Deterministic bucket in `[0, 99]`, or `nil` when no `DeviceID` was supplied. |
| `Eligible` | bool | Whether the install is inside the rollout. When `false`, the SDK has already gated the response. |

`DeviceID` is required to participate: it must be the same stable value used for [telemetry](../../telemetry) (`X-Device-ID`). Without it the bucket cannot be computed, so the install stays out of the rollout (`Eligible: false`, `Bucket: nil`) until a `DeviceID` is provided. Raising the percentage on the same version only ever **adds** installs — an install in the 20% bucket stays in when you move to 50%.
