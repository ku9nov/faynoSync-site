...
Private apps cannot enable `cdn`: manifests are public objects and would expose a private app's versions and links. The API rejects the combination with `400`.
...
When you publish or unpublish **any** version for an app, faynoSync **deletes response manifests** for that application’s scope on object storage. The next checks for affected dimensions are MISSes; the SDK falls back to `/checkVersion`, the API repopulates S3, and subsequent traffic HITs again. The manifest is written in the background after the API has answered, so a request arriving within the next fraction of a second may still MISS.

Deleting a manifest removes it from the bucket, not from caches in front of it. A CDN, or Google Cloud Storage's built-in cache for public objects, can keep serving a deleted or overwritten manifest until its `max-age` (60 seconds) runs out. Plan for up to a minute between a publish and every client seeing it.
