...
- **[Sparkle](/docs/updaters/sparkle)** — reads a materialized appcast, one static file served to every client, so faynoSync's deviceId/percent rollout can't diverge per device. Sparkle's own native `sparkle:phasedRolloutInterval` (time-based, client-side) does pass through the feed verbatim.
