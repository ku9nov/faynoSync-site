...
:::warning Deprecated local storage example
This guide currently uses MinIO-based local storage values for backward compatibility examples. For new local development setups, use Garage with `STORAGE_DRIVER=aws` and the S3 compatibility variables documented in [Environment Configuration](./env-overview).
:::
...
S3_BUCKET_NAME_CDN=cb-faynosync-s3-cdn
...
# TUF (The Update Framework) — optional, enables signed metadata
TUF_ENABLED=true
...
When `TUF_ENABLED=true`, mount the online signing keys into the backend container (add `- ./private_keys:/private_keys` under the `backend` service volumes). Set `TUF_ENABLED=false` to skip TUF signing entirely.

`S3_BUCKET_NAME_CDN` is used for API response caching. You can point it to `S3_BUCKET_NAME` or keep a separate bucket for flexible cache policies.
...
:::tip
**Note**: Change <code>image: ku9nov/faynosync:latest</code> to <code>image: ku9nov/faynosync:<LatestVersion fallbackVersion="v1.6.0" /></code> to get the latest stable release.
:::
...
services:
  backend:
    ...
    command: faynoSync --loglevel=debug
    networks:
      faynosync_network:
  ...
  s3-service:
    ...
    entrypoint: >
      ...
      /usr/bin/mc mb faynoSync/cb-faynosync-s3-cdn;
      /usr/bin/mc anonymous set download faynoSync/cb-faynosync-s3-public;
      "
    env_file:
      - .env.faynosync
```
...
Before deployment, make sure you're using the latest version of FaynoSync. The current latest version is <LatestVersion fallbackVersion="v1.6.0" />. You can check for the latest release on [GitHub](https://github.com/ku9nov/faynoSync/releases).
...
- **FaynoSync Backend** (<LatestVersion fallbackVersion="v1.6.0" />) - The main API service
