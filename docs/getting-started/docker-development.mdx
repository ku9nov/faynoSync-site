...
description: Run FaynoSync locally with Docker and Docker Compose, including API services, dependencies, and development setup.
---
...
For more details, see the [faynoSync README](https://github.com/ku9nov/faynoSync/blob/main/README.md).
...
Once the services are running, you can execute the test suite with the following command (wait until Garage finishes its bootstrap: `docker compose logs s3` shows `Garage bootstrap completed successfully.`):
...
However, Docker isn't ideal for **actively developing the dashboard frontend** — you lose fast hot-reload and a tight edit loop. If you're working on the dashboard UI itself, scale the containerized `frontend` to `0` (see [Running Dependency Services for Local Development](#running-dependency-services-for-local-development) above) and run it from source instead:
