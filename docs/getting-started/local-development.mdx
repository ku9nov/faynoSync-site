...
description: Complete guide to setting up FaynoSync development environment with MongoDB, Golang, and Garage
---
...
You can find full environment variable details and examples in the [Environment Configuration page](./env-overview).
...
Database migrations are a separate CLI action. On first setup (or after schema changes), apply migrations:
...
```bash
go run faynoSync.go migrate down
```
