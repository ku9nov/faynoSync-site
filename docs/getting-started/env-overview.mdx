---
sidebar_position: 1
title: Environment Variables Overview
description: Complete guide to FaynoSync environment variables and configuration options
---

# Environment Variables Overview

To configure `FaynoSync`, you need to set the following environment variables. These settings ensure that your application communicates correctly with storage services, databases, and APIs.

---

## 📋 Quick Reference

| Category | Variables |
|----------|-----------|
| **Core** | `API_URL`, `STORAGE_DRIVER`, `PORT` |
| **Storage** | `S3_*`, `GCS_*`, `MINIO_*` (deprecated) |
| **Database** | `MONGODB_URL`, `MONGODB_URL_TESTS` |
| **Security** | `API_KEY`, `JWT_SECRET` |
| **Features** | `PERFORMANCE_MODE`, `ENABLE_TELEMETRY`, `MAX_BULK_DELETE_VERSIONS` |
| **TUF** | `TUF_ENABLED`, `ONLINE_KEY_DIR` |
| **Reports** | `REPORTS_ENABLED`, `REPORTS_*` |
| **Redis** | `REDIS_*` |
| **Slack** | `SLACK_*` |

---

## 🔧 Required Environment Variables

### Core Configuration

<details>
<summary><strong>API_URL</strong></summary>

Public URL to this API.

**Example:** `API_URL=https://faynosync.example.com`

</details>

<details>
<summary><strong>ENABLE_PRIVATE_APP_DOWNLOADING</strong></summary>

Deprecated. Access to private apps is now set per app with `download_mode` (see [create app](../api/apps/create) and [download](../api/apps/download)). This variable only chooses the default mode: `true` gives new private apps `unlisted` (anyone with the link can download), `false` gives them `strict` (a [download token](../api/apps/download-tokens-regenerate) or a JWT with access is required, on update checks as well as downloads).

On upgrade to v2.3.0, existing private apps get their `download_mode` from this value once. With `false`, their clients must start sending `X-Download-Token`, or the apps must be switched to `unlisted` through [update app](../api/apps/update-app).

**Example:** `ENABLE_PRIVATE_APP_DOWNLOADING=false`

</details>

<details>
<summary><strong>STORAGE_DRIVER</strong></summary>

Specifies the storage driver to use. 

**Available options:**

- **minio**: Deprecated. Kept for backward compatibility with existing setups.
- **aws**: For using `AWS S3` or S3-compatible providers such as local Garage.
- **digitalocean**: For using DigitalOcean Spaces as your object storage solution.
- **gcp**: For using Google Cloud Storage as your object storage solution.

**Example:** `STORAGE_DRIVER=aws`

</details>

<details>
<summary><strong>DASHBOARD_URL</strong></summary>

The URL of your dashboard. This is used for configuring CORS (Cross-Origin Resource Sharing) to allow requests to the auto updater service from your dashboard.

**Example:** `DASHBOARD_URL=http://localhost:3000`

</details>

<details>
<summary><strong>PORT</strong></summary>

The port on which the auto updater service will listen. By default, the service listens on port 9000.

**Example:** `PORT=9000`

</details>

### Database Configuration

<details>
<summary><strong>MONGODB_URL</strong></summary>

The MongoDB connection string used by `FaynoSync`. This string contains information such as the host, database name, and authentication details.

**Example:** `MONGODB_URL=mongodb://root:password@127.0.0.1/faynosync_db?authSource=admin`

</details>

<details>
<summary><strong>MONGODB_URL_TESTS</strong></summary>

The MongoDB connection string used by `FaynoSync` for tests running. This string contains information such as the host, database name, and authentication details.

**Example:** `MONGODB_URL_TESTS=mongodb://root:password@127.0.0.1/faynosync_db_tests?authSource=admin`

</details>

### Security Configuration

<details>
<summary><strong>API_KEY</strong></summary>

This key is used for authentication during sign-up and other API interactions. 

Generate this key using `openssl rand -base64 16`.

**Example:** `API_KEY=generated_api_key`

</details>

<details>
<summary><strong>JWT_SECRET</strong></summary>

A secret key used for signing and verifying JWT (JSON Web Tokens) used by the application for secure communication.

Generate this key using `openssl rand -base64 32`.

**Example:** `JWT_SECRET=your_jwt_secret`

</details>

### Performance & Features

<details>
<summary><strong>PERFORMANCE_MODE</strong></summary>

This setting enables performance optimizations within the application when set to true. Use this for high-load environments where efficiency is a priority.

**Example:** `PERFORMANCE_MODE=true`

</details>

<details>
<summary><strong>ENABLE_TELEMETRY</strong></summary>

This setting enables statistics when set to true. When `false`, `/checkVersion` records no client telemetry and `/telemetry/beacon` returns `403`.

**Example:** `ENABLE_TELEMETRY=true`

</details>

<details>
<summary><strong>MAX_BULK_DELETE_VERSIONS</strong></summary>

Maximum number of version ids accepted by one [`DELETE /apps/delete`](/docs/api/apps/delete-specific-version-app) request. Defaults to `10` when unset or not a positive number.

**Example:** `MAX_BULK_DELETE_VERSIONS=10`

</details>

### TUF Configuration

<details>
<summary><strong>TUF_ENABLED</strong></summary>

Set to `true` to enable TUF (The Update Framework) functionality. This enables secure software update management with metadata signing, artifact publishing, and root key rotation capabilities.

**Example:** `TUF_ENABLED=true`

</details>

<details>
<summary><strong>ONLINE_KEY_DIR</strong></summary>

Directory path where online signing keys are stored. These keys are used for online metadata signing operations in TUF.

**Example:** `ONLINE_KEY_DIR=/private_keys`

</details>

### Reports Configuration

Report ingestion is disabled by default. When enabled, it exposes a public `POST /reports/ingest` endpoint (authenticated by per-app report keys) and the JWT-protected read API. Rate limits are kept in Redis, and detail blobs are stored in the private bucket (`S3_BUCKET_NAME_PRIVATE`) using the existing `STORAGE_DRIVER` / `S3_*` configuration. See [Reports Management](/docs/api/reports/reports-introduction) for the full API.

<details>
<summary><strong>REPORTS_ENABLED</strong></summary>

Set to `true` to enable the report ingestion endpoint and the reports read API. When `false`, none of the report routes are registered.

**Default:** `false`
**Example:** `REPORTS_ENABLED=true`

</details>

<details>
<summary><strong>REPORTS_MAX_BODY_BYTES</strong></summary>

Maximum accepted request body size for `POST /reports/ingest`, in bytes. Requests over this limit return `413 Payload Too Large`.

**Default:** `262144` (256 KB)
**Example:** `REPORTS_MAX_BODY_BYTES=262144`

</details>

<details>
<summary><strong>REPORTS_MAX_DETAILS_COMPRESSED_BYTES</strong></summary>

Maximum size of the compressed (`gzip+base64`-decoded) details payload, in bytes. Larger payloads return `413`.

**Default:** `131072` (128 KB)
**Example:** `REPORTS_MAX_DETAILS_COMPRESSED_BYTES=131072`

</details>

<details>
<summary><strong>REPORTS_MAX_DETAILS_DECOMPRESSED_BYTES</strong></summary>

Maximum size of the details payload after gzip decompression, in bytes. Enforced with a limited reader (zip-bomb guard); the client-declared size is never trusted. Exceeding it returns `413`.

**Default:** `1048576` (1 MB)
**Example:** `REPORTS_MAX_DETAILS_DECOMPRESSED_BYTES=1048576`

</details>

<details>
<summary><strong>REPORTS_BLOB_RETENTION_DAYS</strong></summary>

How long detail blobs are retained. Each `report_blobs` document gets an `expiresAt` of `createdAt + REPORTS_BLOB_RETENTION_DAYS` and is removed by a TTL index. Changing this only affects future blobs.

**Default:** `30`
**Example:** `REPORTS_BLOB_RETENTION_DAYS=30`

</details>

<details>
<summary><strong>REPORTS_MAX_BLOBS_PER_GROUP</strong></summary>

Maximum number of detail blobs kept per report group. When a new blob is stored beyond this limit, the oldest blobs (and their storage objects) are removed.

**Default:** `10`
**Example:** `REPORTS_MAX_BLOBS_PER_GROUP=10`

</details>

<details>
<summary><strong>REPORTS_STORAGE_PREFIX</strong></summary>

Key prefix used for report detail blobs in the private bucket (for example `reports/<app>/<date>/...`).

**Default:** `reports`
**Example:** `REPORTS_STORAGE_PREFIX=reports`

</details>

<details>
<summary><strong>REPORTS_RATE_LIMIT_PER_KEY_PER_MINUTE</strong></summary>

Per-report-key rate limit for ingestion, in requests per minute. Hitting any rate limit returns `429 Too Many Requests`. (Per-device and per-group limits are fixed and not configurable.)

**Default:** `100`
**Example:** `REPORTS_RATE_LIMIT_PER_KEY_PER_MINUTE=100`

</details>

### Redis Configuration

Redis is required. The API connects on startup, regardless of which features are enabled, and exits if Redis is unreachable.

<details>
<summary><strong>REDIS_HOST</strong></summary>

The hostname or IP address of the Redis server. If not specified, it defaults to `localhost`.

**Example:** `REDIS_HOST=your_redis_host`

</details>

<details>
<summary><strong>REDIS_PORT</strong></summary>

The port number on which the Redis server is listening. The default value is 6379.

**Example:** `REDIS_PORT=6379`

</details>

<details>
<summary><strong>REDIS_PASSWORD</strong></summary>

The password for authenticating with the Redis server. Leave this empty if no password is set for your Redis instance.

**Example:** `REDIS_PASSWORD=your_redis_password`

</details>

<details>
<summary><strong>REDIS_DB</strong></summary>

Specifies which Redis database number to use. The default is 0.

**Example:** `REDIS_DB=0`

</details>

### Slack Integration

<details>
<summary><strong>SLACK_ENABLE</strong></summary>

Specifies whether Slack integration is enabled. If `SLACK_ENABLE=false`, no messages will be sent to Slack. The default is `false`.

**Example:** `SLACK_ENABLE=true`

</details>

<details>
<summary><strong>SLACK_BOT_TOKEN</strong></summary>

The authentication token for the Slack bot used to access the API. It should be in the format xoxb-*.

**Example:** `SLACK_BOT_TOKEN=xoxb-123456789012-1234567890123-abcdef123456`

</details>

<details>
<summary><strong>SLACK_CHANNEL</strong></summary>

Specifies the name of the Slack channel where messages will be sent.

**Example:** `SLACK_CHANNEL=test-channel`

</details>

<details>
<summary><strong>SLACK_NOTIFICATION_TTL</strong></summary>

Configures how long Redis-backed Slack notification state is kept for a given app version.  
Slack notifications reuse a single mutable message per app version across artifact create, update, and delete flows.

**Default:** `24h`  
**Example:** `SLACK_NOTIFICATION_TTL=24h`

</details>

---

## 🗄️ Storage Configuration

The storage client is created once per process and reused. Storage credentials (`S3_ACCESS_KEY` / `S3_SECRET_KEY`, `GCS_CREDENTIALS_FILE`) and `STORAGE_DRIVER` are read at that point, so restart faynoSync after rotating a key. On GCS, the bucket's uniform bucket-level access setting is also read once per bucket; restart after changing it.

### MinIO (Deprecated)

MinIO is deprecated for new setups and is kept for backward compatibility only.
For local development, use Garage with the `aws` storage driver and `S3_*` compatibility options from the AWS S3 section below.

For legacy MinIO deployments only:

| Variable | Description | Example |
|----------|-------------|---------|
| `STORAGE_DRIVER` | Set to `minio` for MinIO storage (deprecated). | `STORAGE_DRIVER=minio` |
| `MINIO_SECURE` | Specifies whether to use secure connections (HTTPS) for MinIO. Set to `false` for local development. | `MINIO_SECURE=false` |
| `S3_ACCESS_KEY` | Your MinIO access key ID. | `S3_ACCESS_KEY=your_minio_access_key` |
| `S3_SECRET_KEY` | Your MinIO secret access key. | `S3_SECRET_KEY=your_minio_secret_key` |
| `S3_BUCKET_NAME_PRIVATE` | The name of your MinIO bucket for private storage. | `S3_BUCKET_NAME_PRIVATE=cb-faynosync-s3-private` |
| `S3_BUCKET_NAME` | The name of your MinIO bucket for public storage. | `S3_BUCKET_NAME=cb-faynosync-s3-public` |
| `S3_BUCKET_NAME_CDN` | Bucket used for API response caching. You can reuse `S3_BUCKET_NAME` or use a dedicated bucket for flexible cache settings. | `S3_BUCKET_NAME_CDN=cb-faynosync-s3-public` |
| `S3_ENDPOINT_PRIVATE` | The MinIO endpoint URL. | `S3_ENDPOINT_PRIVATE=localhost:9010` |
| `S3_ENDPOINT` | The public MinIO endpoint URL. | `S3_ENDPOINT=localhost:9010` |

### AWS S3 (Amazon Simple Storage Service)

For using AWS S3 as your object storage solution:

| Variable | Description | Example |
|----------|-------------|---------|
| `STORAGE_DRIVER` | Set to `aws` for AWS S3 storage. | `STORAGE_DRIVER=aws` |
| `S3_ACCESS_KEY` | Your AWS access key ID. | `S3_ACCESS_KEY=your_aws_access_key` |
| `S3_SECRET_KEY` | Your AWS secret access key. | `S3_SECRET_KEY=your_aws_secret_key` |
| `S3_REGION` | The AWS region in which your S3 bucket is located. | `S3_REGION=us-east-1` |
| `S3_BUCKET_NAME_PRIVATE` | The name of your AWS S3 bucket for private storage. | `S3_BUCKET_NAME_PRIVATE=your-private-bucket` |
| `S3_BUCKET_NAME` | The name of your AWS S3 bucket for public storage. | `S3_BUCKET_NAME=your-public-bucket` |
| `S3_BUCKET_NAME_CDN` | Bucket used for API response caching. You can reuse `S3_BUCKET_NAME` or use a dedicated bucket for flexible cache settings. | `S3_BUCKET_NAME_CDN=your-public-bucket` |
| `S3_ENDPOINT_PRIVATE` | The AWS S3 endpoint URL. | `S3_ENDPOINT_PRIVATE=https://your-bucket-name.s3.amazonaws.com` |
| `S3_ENDPOINT` | The public AWS S3 endpoint URL. | `S3_ENDPOINT=https://your-public-bucket.s3.amazonaws.com` |
| `S3_API_ENDPOINT` | Optional custom S3 API endpoint for S3-compatible providers such as local Garage. Leave empty for managed AWS S3. | `S3_API_ENDPOINT=http://localhost:3900` |
| `S3_FORCE_PATH_STYLE` | Optional. Set to `true` for providers that require path-style requests, such as local Garage. | `S3_FORCE_PATH_STYLE=true` |
| `S3_DISABLE_OBJECT_ACL` | Optional. Set to `true` for providers that do not support `public-read` object ACLs, such as local Garage. | `S3_DISABLE_OBJECT_ACL=true` |

### DigitalOcean Spaces (S3-compatible Storage from DigitalOcean)

For using DigitalOcean Spaces as your object storage solution:

| Variable | Description | Example |
|----------|-------------|---------|
| `STORAGE_DRIVER` | Set to `digitalocean` for DigitalOcean Spaces storage. | `STORAGE_DRIVER=digitalocean` |
| `S3_ACCESS_KEY` | Your DigitalOcean Spaces access key. | `S3_ACCESS_KEY=DO00EXAMPLEKEY` |
| `S3_SECRET_KEY` | Your DigitalOcean Spaces secret key. | `S3_SECRET_KEY=EXAMPLE_SECRET_KEY` |
| `S3_REGION` | The DigitalOcean region where your Space is located. | `S3_REGION=nyc3` |
| `S3_BUCKET_NAME_PRIVATE` | The name of your DigitalOcean Space for private storage. | `S3_BUCKET_NAME_PRIVATE=your-private-space` |
| `S3_BUCKET_NAME` | The name of your DigitalOcean Space for public storage. | `S3_BUCKET_NAME=your-public-space` |
| `S3_BUCKET_NAME_CDN` | Bucket used for API response caching. You can reuse `S3_BUCKET_NAME` or use a dedicated bucket for flexible cache settings. | `S3_BUCKET_NAME_CDN=your-public-space` |
| `S3_ENDPOINT_PRIVATE` | The DigitalOcean Spaces endpoint URL. | `S3_ENDPOINT_PRIVATE=nyc3.digitaloceanspaces.com` |

### Google Cloud Storage (GCP)

For using Google Cloud Storage as your object storage solution:

| Variable | Description | Example |
|----------|-------------|---------|
| `STORAGE_DRIVER` | Set to `gcp` for Google Cloud Storage. | `STORAGE_DRIVER=gcp` |
| `GCS_CREDENTIALS_FILE` | Path to Google Cloud service account credentials JSON file. | `GCS_CREDENTIALS_FILE=/path/to/credentials.json` |
| `GCS_SERVICE_ACCOUNT_EMAIL` | GCS service account email for presigned URL generation. | `GCS_SERVICE_ACCOUNT_EMAIL=your-service-account@project.iam.gserviceaccount.com` |
| `GCS_PRIVATE_KEY` | GCS private key for presigned URL generation. | `GCS_PRIVATE_KEY=-----BEGIN PRIVATE KEY-----\nMIIEv...\n-----END PRIVATE KEY-----\n` |
| `S3_BUCKET_NAME_PRIVATE` | The name of your GCS bucket for private storage. | `S3_BUCKET_NAME_PRIVATE=your-private-gcs-bucket` |
| `S3_BUCKET_NAME` | The name of your GCS bucket for public storage. | `S3_BUCKET_NAME=your-public-gcs-bucket` |
| `S3_BUCKET_NAME_CDN` | Bucket used for API response caching. You can reuse `S3_BUCKET_NAME` or use a dedicated bucket for flexible cache settings. | `S3_BUCKET_NAME_CDN=your-public-gcs-bucket` |

### Presigned Uploads

Applies to every storage driver that supports [presigned uploads](/docs/presigned-uploads) (AWS S3, DigitalOcean Spaces, Garage, GCS).

| Variable | Description | Example |
|----------|-------------|---------|
| `PRESIGNED_UPLOAD_URL_TTL` | Lifetime of the presigned PUT URLs returned by [Upload Init](/docs/api/apps/upload-init), as a Go duration. Default `30m`, maximum `168h` (7 days, the signed URL limit of S3-compatible storage, applied to every driver). An invalid or out-of-range value falls back to `30m`. The pending upload record lives 90 minutes longer than the URLs. | `PRESIGNED_UPLOAD_URL_TTL=2h` |

---

## ⚙️ Setting Up Environment Variables

You can set these environment variables in a `.env` file in the root directory of the application. Here's an example of what your `.env` file might look like:

```bash
# =============================================================================
# MINIO (deprecated, kept for backward compatibility)
# =============================================================================
# STORAGE_DRIVER=minio
# MINIO_SECURE=false
S3_BUCKET_NAME_PRIVATE=cb-faynosync-s3-private
S3_ENDPOINT_PRIVATE=localhost:9010
S3_ACCESS_KEY=
S3_SECRET_KEY=
S3_BUCKET_NAME=cb-faynosync-s3-public
S3_BUCKET_NAME_CDN=cb-faynosync-s3-public
S3_ENDPOINT=localhost:9010

# =============================================================================
# AWS S3 (Amazon Simple Storage Service)
# =============================================================================
STORAGE_DRIVER=aws
#S3_ACCESS_KEY=
#S3_SECRET_KEY=
#S3_REGION=
# S3_BUCKET_NAME_PRIVATE=
# S3_BUCKET_NAME=
# S3_BUCKET_NAME_CDN= # can be the same as S3_BUCKET_NAME, or a dedicated cache bucket
#S3_ENDPOINT_PRIVATE=https://<bucket_name>.s3.amazonaws.com # for example in us-east-1
#S3_ENDPOINT=https://<bucket_name>.s3.amazonaws.com # for example in us-east-1
#S3_API_ENDPOINT=http://localhost:3900 # optional for S3-compatible providers (Garage, etc.)
#S3_FORCE_PATH_STYLE=true # optional for providers that require path-style requests
#S3_DISABLE_OBJECT_ACL=true # optional for providers without public-read ACL support

# =============================================================================
# DIGITALOCEAN SPACES (S3-compatible storage from DigitalOcean)
# =============================================================================
# STORAGE_DRIVER=digitalocean
# S3_ENDPOINT_PRIVATE=nyc3.digitaloceanspaces.com
# S3_REGION=nyc3
# S3_ACCESS_KEY=DO00EXAMPLEKEY
# S3_SECRET_KEY=EXAMPLE_SECRET_KEY
# S3_BUCKET_NAME_PRIVATE=
# S3_BUCKET_NAME=
# S3_BUCKET_NAME_CDN= # can be the same as S3_BUCKET_NAME, or a dedicated cache bucket

# =============================================================================
# GOOGLE CLOUD STORAGE (GCP)
# =============================================================================
# STORAGE_DRIVER=gcp
# GCS_CREDENTIALS_FILE=/path/to/credentials.json
# GCS_SERVICE_ACCOUNT_EMAIL=your-service-account@project.iam.gserviceaccount.com
# GCS_PRIVATE_KEY=-----BEGIN PRIVATE KEY-----\nMIIEv...\n-----END PRIVATE KEY-----\n
# S3_BUCKET_NAME_PRIVATE=
# S3_BUCKET_NAME=
# S3_BUCKET_NAME_CDN= # can be the same as S3_BUCKET_NAME, or a dedicated cache bucket

# =============================================================================
# Other Configuration
# =============================================================================
DASHBOARD_URL=http://localhost:3000
PORT=9000
MONGODB_URL=mongodb://root:password@127.0.0.1/faynosync_db?authSource=admin
API_KEY=generated_api_key
API_URL=http://localhost:9000
ENABLE_PRIVATE_APP_DOWNLOADING=false
JWT_SECRET=your_jwt_secret
MONGODB_URL_TESTS=mongodb://root:password@127.0.0.1/faynosync_db_tests?authSource=admin
PERFORMANCE_MODE=true
REDIS_HOST=localhost
REDIS_PORT=6379
REDIS_PASSWORD=
REDIS_DB=0
SLACK_ENABLE=false
SLACK_BOT_TOKEN=xoxb-*
SLACK_CHANNEL=test-channel
SLACK_NOTIFICATION_TTL=24h
ENABLE_TELEMETRY=true
MAX_BULK_DELETE_VERSIONS=10

# =============================================================================
# TUF (The Update Framework)
# =============================================================================
TUF_ENABLED=true
ONLINE_KEY_DIR=/private_keys

# =============================================================================
# Reports (client failure report ingestion)
# =============================================================================
REPORTS_ENABLED=false
REPORTS_MAX_BODY_BYTES=262144
REPORTS_MAX_DETAILS_COMPRESSED_BYTES=131072
REPORTS_MAX_DETAILS_DECOMPRESSED_BYTES=1048576
REPORTS_BLOB_RETENTION_DAYS=30
REPORTS_MAX_BLOBS_PER_GROUP=10
REPORTS_STORAGE_PREFIX=reports
REPORTS_RATE_LIMIT_PER_KEY_PER_MINUTE=100
```

For convenience, you can also use the `.env.local` file, which contains all the variables already filled in. Ensure that your environment variables are properly set before running the application to avoid misconfigurations.

