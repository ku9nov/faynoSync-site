...
Redis is required. faynoSync stores response cache (performance mode), telemetry counters, TUF task and signing state, rate limits and Slack notification state in it. The API exits on startup if Redis is unreachable.
...
- Reuse your **public** bucket for `S3_BUCKET_NAME_CDN`, or create a dedicated **cdn** bucket for more flexible cache settings.
...
S3_BUCKET_NAME_CDN=your-public-bucket  # can match S3_BUCKET_NAME or use a dedicated cache bucket
...
MAX_BULK_DELETE_VERSIONS=10  # ids per DELETE /apps/delete request
...
# Security
ENABLE_PRIVATE_APP_DOWNLOADING=true  # default download_mode for new private apps: true = unlisted, false = strict
...
# Reports (optional; blobs stored in S3_BUCKET_NAME_PRIVATE)
REPORTS_ENABLED=false  # or true
...
:::info
When `TUF_ENABLED=true`, `ONLINE_KEY_DIR` must point to a directory (inside the container) holding the online signing keys. Mount it into the container in your `docker-compose.yaml` (see below). Leave `TUF_ENABLED=false` if you don't use TUF signing.
:::
...
:::tip
**Note**: Change <code>image: ku9nov/faynosync:latest</code> to <code>image: ku9nov/faynosync:<LatestVersion fallbackVersion="v1.6.0" /></code> to use the latest stable release.
:::
...
services:
  backend:
    ...
    volumes:
      -  ./.env:/app/.env
      # Required only when TUF_ENABLED=true — mount the online signing keys
